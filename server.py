...
Security features:
  - 2FA via email (6-digit code on login)
  - Forgot password with email verification
  - Security alerts (login, password reset, failed attempts) to admin
"""
...
# Security email — where 2FA codes + reset links go
SECURITY_EMAIL = "brad@mpamotorsports.com"
# Admin email — where login/reset/failed attempt alerts go
ADMIN_EMAIL = "nexaraaisaadeh@gmail.com"

# Dashboard URL for reset links
DASHBOARD_URL = os.environ.get("DASHBOARD_URL", "https://mpa-render.onrender.com")

# ── Email Sender (background thread to avoid blocking) ────────────────
def send_email(to_email: str, subject: str, html_body: str):
    """Send email via Gmail SMTP in background thread."""
    if not SMTP_EMAIL or not SMTP_PASSWORD:
        print(f"[EMAIL SKIPPED] No SMTP credentials. Would send to {to_email}: {subject}")
        return

    def _send():
        try:
            msg = MIMEMultipart("alternative")
            msg["From"] = f"MPA Motorsports <{SMTP_EMAIL}>"
            msg["To"] = to_email
            msg["Subject"] = subject
...
def send_2fa_code(to_email: str, code: str, username: str):
    """Send 2FA verification code."""
    send_email(to_email, f"MPA Dashboard — Your Login Code: {code}", f"""
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;">
      <h2 style="color:#1a1a2e;margin-bottom:8px;">MPA Motorsports Dashboard</h2>
      <p style="color:#555;margin-bottom:24px;">Verification code for <strong>{username}</strong></p>
...
def send_reset_email(to_email: str, reset_url: str, username: str):
    """Send password reset link."""
    send_email(to_email, "MPA Dashboard — Password Reset", f"""
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;">
      <h2 style="color:#1a1a2e;margin-bottom:8px;">MPA Motorsports Dashboard</h2>
      <p style="color:#555;margin-bottom:24px;">Password reset request for <strong>{username}</strong></p>
      <a href="{reset_url}" style="display:inline-block;background:#3b82f6;color:#fff;padding:12px 32px;border-radius:8px;text-decoration:none;font-weight:bold;margin-bottom:24px;">Reset Password</a>
...
def send_admin_alert(subject: str, body_html: str):
    """Send security alert to admin."""
    send_email(ADMIN_EMAIL, subject, f"""
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;">
      <h2 style="color:#1a1a2e;margin-bottom:8px;">🔒 MPA Security Alert</h2>
      {body_html}
      <p style="color:#aaa;font-size:12px;margin-top:24px;">MPA Motorsports Dashboard — Automated Security Alert</p>
    </div>
    """)
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS users (
        ...
        email TEXT,
        security_email TEXT DEFAULT 'brad@mpamotorsports.com',
    ...
    # Seed users — both use brad@mpamotorsports.com as security email
    users = [
        ("ryan", hash_pw("1w3a&r6hQ"), "Ryan", "ryan@mpamotorsports.com", "brad@mpamotorsports.com", "owner"),
        ("brad", hash_pw("te@Mh05f0"), "Brad", "brad@mpamotorsports.com", "brad@mpamotorsports.com", "staff"),
    ]
    for u in users:
        try:
            db.execute("INSERT INTO users (username, password_hash, display_name, email, security_email, role) VALUES (?,?,?,?,?,?)", u)
...
class ForgotPasswordRequest(BaseModel):
    username: str
    email: str  # Must match the security email on file
...
# ── Auth — Step 1: Login (username + password → sends 2FA code) ──────
...
def login(req: LoginRequest, request: Request):
    ...
    if not user or user["password_hash"] != hash_pw(req.password):
        ...
        db.execute("INSERT INTO failed_logins (username, ip_address) VALUES (?,?)",
                   (req.username.lower().strip(), ip))
        db.commit()

        # Count recent failures for this username
        count = db.execute(
            "SELECT COUNT(*) FROM failed_logins WHERE username = ? AND created_at > datetime('now', '-1 hour')",
            (req.username.lower().strip(),)
        ).fetchone()[0]
        db.close()

        # Alert admin on every failed attempt
        send_admin_alert(
            f"⚠️ Failed Login Attempt — MPA Dashboard",
            f"""
            <p style="color:#e53e3e;font-weight:bold;">Failed login attempt detected</p>
            <table style="width:100%;border-collapse:collapse;margin:12px 0;">
              <tr><td style="padding:6px 0;color:#888;">Username:</td><td style="padding:6px 0;font-weight:bold;">{req.username}</td></tr>
              <tr><td style="padding:6px 0;color:#888;">IP Address:</td><td style="padding:6px 0;font-family:monospace;">{ip}</td></tr>
              <tr><td style="padding:6px 0;color:#888;">Time (UTC):</td><td style="padding:6px 0;">{datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}</td></tr>
              <tr><td style="padding:6px 0;color:#888;">Attempts (last hour):</td><td style="padding:6px 0;font-weight:bold;color:#e53e3e;">{count}</td></tr>
            </table>
            """
        )
    ...
    db.execute("INSERT INTO two_factor_codes (user_id, code, pending_token, expires_at) VALUES (?,?,?,?)",
               (user["id"], code, pending_token, expires.isoformat()))
    db.commit()
    db.close()

    # Send 2FA code to security email
    send_2fa_code(SECURITY_EMAIL, code, user["username"])
...
def verify_2fa(req: Verify2FARequest, request: Request):
    ...
    row = db.execute("""
        SELECT tf.*, u.id as uid, u.username, u.display_name, u.role, u.email
        FROM two_factor_codes tf JOIN users u ON tf.user_id = u.id
        WHERE tf.pending_token = ? AND tf.used = 0 AND tf.expires_at > datetime('now')
    """, (req.pending_token,)).fetchone()
    ...
    db.execute("UPDATE two_factor_codes SET used = 1 WHERE id = ?", (row["id"],))

    # Create session
    token = secrets.token_hex(32)
    expires = datetime.now() + timedelta(hours=24)
    db.execute("INSERT INTO sessions (token, user_id, expires_at) VALUES (?,?,?)",
               (token, row["uid"], expires.isoformat()))
    db.commit()
    db.close()

    # Alert admin about successful login
    send_admin_alert(
        f"✅ Login — {row['display_name']} signed in",
        f"""
        <p style="color:#38a169;font-weight:bold;">Successful login</p>
        <table style="width:100%;border-collapse:collapse;margin:12px 0;">
          <tr><td style="padding:6px 0;color:#888;">User:</td><td style="padding:6px 0;font-weight:bold;">{row['display_name']} ({row['username']})</td></tr>
          <tr><td style="padding:6px 0;color:#888;">Role:</td><td style="padding:6px 0;">{row['role']}</td></tr>
          <tr><td style="padding:6px 0;color:#888;">IP Address:</td><td style="padding:6px 0;font-family:monospace;">{ip}</td></tr>
          <tr><td style="padding:6px 0;color:#888;">Time (UTC):</td><td style="padding:6px 0;">{datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}</td></tr>
        </table>
        """
...
# ── Forgot Password (requires email verification) ────────────────────
...
def forgot_password(req: ForgotPasswordRequest):
    ...
    if not user:
        ...
        return {"ok": True, "message": "If the account and email match, a reset link has been sent."}

    # Verify security email matches
    if user["security_email"] and user["security_email"].lower() != req.email.lower().strip():
        db.close()
        return {"ok": True, "message": "If the account and email match, a reset link has been sent."}
    ...
    send_reset_email(SECURITY_EMAIL, reset_url, user["username"])
...
def reset_password(req: ResetPasswordRequest, request: Request):
    ...
    # Alert admin about password reset
    send_admin_alert(
        f"🔑 Password Reset — {reset['display_name']}",
        f"""
        <p style="color:#dd6b20;font-weight:bold;">Password was reset</p>
        <table style="width:100%;border-collapse:collapse;margin:12px 0;">
          <tr><td style="padding:6px 0;color:#888;">User:</td><td style="padding:6px 0;font-weight:bold;">{reset['display_name']} ({reset['username']})</td></tr>
          <tr><td style="padding:6px 0;color:#888;">IP Address:</td><td style="padding:6px 0;font-family:monospace;">{ip}</td></tr>
          <tr><td style="padding:6px 0;color:#888;">Time (UTC):</td><td style="padding:6px 0;">{datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}</td></tr>
        </table>
        """
